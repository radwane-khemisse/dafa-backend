...
class ProductUpsell(Base):
    __tablename__ = "product_upsells"
    __table_args__ = (UniqueConstraint("source_product_id", "target_product_id", "market_code", name="uq_product_upsell"),)

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    source_product_id: Mapped[str] = mapped_column(String(80), nullable=False, index=True)
    target_product_id: Mapped[str] = mapped_column(String(80), nullable=False, index=True)
    market_code: Mapped[str] = mapped_column(String(10), nullable=False, index=True)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())


class OfferMarketPrice(Base):
    __tablename__ = "offer_market_prices"
    __table_args__ = (UniqueConstraint("product_id", "offer_id", "market_code", name="uq_offer_market_price"),)

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    product_id: Mapped[str] = mapped_column(String(80), nullable=False, index=True)
    offer_id: Mapped[str] = mapped_column(String(40), nullable=False, index=True)
    market_code: Mapped[str] = mapped_column(String(10), nullable=False, index=True)
